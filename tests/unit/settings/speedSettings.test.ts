...
describe('SpeedSettings.replayWpm', () => {
  it('returns the base WPM unchanged when REPLAY_MULTIPLIER is 1.0', () => {
    expect(SpeedSettings.replayWpm(20)).toBe(20)
    expect(SpeedSettings.replayWpm(30)).toBe(30)
    expect(SpeedSettings.replayWpm(23)).toBe(23)
  })

  it('enforces a minimum of 1 for zero or negative inputs', () => {
    expect(SpeedSettings.replayWpm(0)).toBe(1)
    expect(SpeedSettings.replayWpm(-5)).toBe(1)
  })

  it('rounds fractional inputs', () => {
    // 1.0 * 20.7 = 20.7, rounds to 21
    expect(SpeedSettings.replayWpm(20.7)).toBe(21)
    expect(SpeedSettings.replayWpm(20.2)).toBe(20)
  })

  it('returns 1 for very small positive values', () => {
    expect(SpeedSettings.replayWpm(0.4)).toBe(1)
  })
})

describe('SpeedSettings.REPLAY_MULTIPLIER', () => {
  it('is exactly 1.0', () => {
    expect(SpeedSettings.REPLAY_MULTIPLIER).toBe(1.0)
  })

  it('is a static readonly constant', () => {
    // Ensure the value is accessible as a class constant
    expect(typeof SpeedSettings.REPLAY_MULTIPLIER).toBe('number')
  })
})

describe('SpeedSettings.applySpeedRacer replay always at 1.0×', () => {
  let s: SpeedSettings
  beforeEach(() => {
    s = createSpeedSettings()
    s.speedRacerEnabled(true)
  ...
  it('replay uses 1.0× even when first multiplier is greater than 1.0', () => {
    // mults[0] = 2.0, but replay should still be 1.0× (20 wpm), not 40 wpm
    s.speedRacerMultipliers('2.0, 1.5')
    const base = new ApplicableSpeed(20, 12)
    expect(s.applySpeedRacer(base, 0, 3).wpm).toBe(40) // 20 * 2.0
    expect(s.applySpeedRacer(base, 1, 3).wpm).toBe(30) // 20 * 1.5
    expect(s.applySpeedRacer(base, 2, 3).wpm).toBe(20) // replay at 1.0×, not 2.0×
  })

  it('replay uses 1.0× even when first multiplier is less than 1.0', () => {
    // mults[0] = 0.5 (slow), but replay should still be 1.0× (20 wpm), not 10 wpm
    s.speedRacerMultipliers('0.5, 0.75')
    const base = new ApplicableSpeed(20, 20)
    expect(s.applySpeedRacer(base, 0, 3).wpm).toBe(10) // 20 * 0.5
    expect(s.applySpeedRacer(base, 1, 3).wpm).toBe(15) // 20 * 0.75
    expect(s.applySpeedRacer(base, 2, 3).wpm).toBe(20) // replay at 1.0×, not 0.5×
  })

  it('preview final WPM is always baseWpm, independent of multiplier list', () => {
    s.speedRacerEnabled(true)
    s.trueWpm(25)
    s.speedRacerSpeakBeforeReplay(false)
    // Multipliers that would produce different values if mults[0] were used
    s.speedRacerMultipliers('2.0, 1.5')
    expect(s.speedRacerPreview()).toBe('50 → 38 → 25 wpm')
    s.speedRacerMultipliers('0.5, 0.75')
    expect(s.speedRacerPreview()).toBe('13 → 19 → 25 wpm')
  })

  it('returns base when playIndex is negative', () => {
    s.speedRacerMultipliers('1.5, 1.0')
    const base = new ApplicableSpeed(20, 12)
    expect(s.applySpeedRacer(base, -1, 3)).toBe(base)
  })
})
