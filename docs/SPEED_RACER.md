...
**Keep Lines:** When **Keep Lines** is on, each newline-delimited sentence is one card. Speed Racer runs its full multiplier ladder once per card (the whole sentence at each step), not once per word inside the sentence.
